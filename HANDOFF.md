...
- ✅ [iter270] **bias-trend.test.ts の TimeEntry fixture を schema 一致に揃え as cast 撤廃 (refactor, 1 commit)**: iter269b 並行 agent が追加した bias-trend.test.ts の fixture は schema に存在しない `idempotencyKey` を入れていて型不整合のため `as unknown as TimeEntry` 二重 cast に頼っていた (judge.sh の `recent-any-leak(=1)` interrupt がこれを検出)。fixture を TimeEntry schema 一致 (不正 `idempotencyKey` 削除、欠けていた `syncStatus` / `syncAttempts` / `syncError` / `externalRef` / `version` 追加) に揃えて `as unknown as` 撤廃 → 型直接代入で OK。tests 15/15 PASS、typecheck/lint 緑。副次効果: detect-patterns の `recent-any-leak` 1→0 で interrupt 判定 noise が消えた。bias-selector.test.ts / format.test.ts / iter261b と同じ「fixture を schema 一致」パターン踏襲。次 iter271 = 21%5=1 → basics。残 large file は item-edit-dialog.tsx 691 / item/service.ts 704 / sprints-panel.tsx 744 / workflows-panel.tsx 760 — 次 refactor iter275 で。
