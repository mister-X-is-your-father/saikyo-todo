...
- ✅ [iter273] **Today EmptyState の説明文を自然言語入力例に置き換え (basics, 1 commit)**: 旧説明「scheduled_for / dueDate を設定すると Today に出てきます」は schema 用語で一般ユーザに不親切だった → QuickAdd の自然言語入力例 3 つ (`明日 資料準備` JA / `tomorrow 9am review` EN / `3/15 リリース` M-D US) を `<code>` chip で示す形に置き換え、新規ユーザでも「どう入力すればここに出るか」が一目で分かるように。EmptyState の `description` 型を `string` → `ReactNode` に広げて JSX 受理 (既存 string 受け callee は代入互換で無変更)。iter256/258/266/271b で nl-parse に追加した複数表記を実例で促す効果も。typecheck/lint 緑 (warning baseline 1)。次 iter274 = 24%5=4 → ai-automation。残候補: bias-brief-rich を Slack DM / pm-agent で実消費、ItemEditDialog にも bias trend / 校正 chip 横展開、AI 朝 brief job 作成、Inbox / Backlog の EmptyState も自然言語例に揃える。
