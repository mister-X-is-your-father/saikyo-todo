/**
 * Phase 6.15 loop iter 983 — integrations/page.tsx main aria-label。workspace sweep 7/8。
 */
...
async function main(): Promise<void> {
  ...
  if (!/aria-label="API 連携 外部 API \(Yamory \/ カスタム REST\) → Item 取込"/.test(src)) {
    findings.push({ level: 'warning', message: `${target}: aria-label 不在` })
  } else {
    findings.push({ level: 'info', message: `integrations aria-label OK` })
  }
  ...
  console.log(`\n=== Findings (integrations-page-main-aria-label-iter983) ===`)
