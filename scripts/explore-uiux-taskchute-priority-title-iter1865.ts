/**
 * Phase 6.15 loop iter1865: taskchute-view priority chip に title 付与
 * (today/inbox/period priority dot と統一)。
 */
...
async function main() {
  ...
  if (!taskchute.includes('title={`P${item.priority ?? 4}`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'taskchute priority chip title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — taskchute priority chip title 付与、iter1863-1777 invariant 不変')
  } else {
