/**
 * Phase 6.15 loop iter1863: personal-period-view priority dot に title 付与
 * (today-view priority dot title=`p${priority}` と統一)。
 */
...
async function main() {
  ...
  if (!period.includes('title={`p${it.priority ?? 4}`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'period view priority dot title が無い',
    })
  }

  const today = readFileSync(resolve(here, '../src/components/workspace/today-view.tsx'), 'utf8')
  if (!today.includes('title={`p${it.priority ?? 4}`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'today-view priority dot title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — period priority dot title 付与、iter1861-1777 invariant 不変')
  } else {
