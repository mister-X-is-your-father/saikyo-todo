/**
 * Phase 6.15 loop iter1963: workspace nav landmark に title 付与
 * (iter1779 個別 nav link title の続編、nav landmark の summary を hover で disclose)。
 */
...
async function main() {
  ...
  if (
    ...
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'workspace nav landmark title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — workspace nav landmark title 付与、iter1961-1777 invariant 不変')
  } else {
