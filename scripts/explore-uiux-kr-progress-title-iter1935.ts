/**
 * Phase 6.15 loop iter1935: goals-panel KR progressbar に title 付与
...
async function main() {
  ...
  if (!gp.includes('title={`KR「${kr.title}」進捗 ${pct}%`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'goals-panel KR progressbar title が無い',
    })
  }
  if (!gp.includes('title={`Goal「${goal.title}」全体進捗 ${goalPct}%')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'iter1933 goals-panel progressbar title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — goals-panel KR progressbar title 付与、iter1933-1777 invariant 不変')
  } else {
