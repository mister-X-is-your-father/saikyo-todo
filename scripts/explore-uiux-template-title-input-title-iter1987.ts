/**
 * Phase 6.15 loop iter1987: template-items-editor 子 Item タイトル input に title 付与
 * (10 state-dependent input family の続編)。
 */
...
async function main() {
  ...
  if (!tmpl.includes('iter1987') || !tmpl.includes('子 Item のタイトル (必須、最大 500 文字')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'template-items 子 Item タイトル input title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — template-items 子 Item タイトル title 付与、iter1985-1777 invariant 不変')
  } else {
