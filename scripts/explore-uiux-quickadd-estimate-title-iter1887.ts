/**
 * Phase 6.15 loop iter1887: quick-add estimate chip に title 付与
 * (subtasks step/childcount / team-capacity title sweep の続編)。
 */
...
async function main() {
  ...
  if (!qa.includes('title={`${formatEstimate(preview.estimateMinutes)} — 見積`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'quick-add estimate chip title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — quick-add estimate title 付与、iter1885-1777 invariant 不変')
  } else {
