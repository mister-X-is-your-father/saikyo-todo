/**
 * Phase 6.15 loop iter 982 — templates/page.tsx main aria-label。workspace sweep 6/8。
 */
...
async function main(): Promise<void> {
  ...
  const target = 'src/app/(workspace)/[workspaceId]/templates/page.tsx'
  const src = readFileSync(resolve(process.cwd(), target), 'utf8')
  if (!/aria-label="Templates ワークパッケージ定義"/.test(src)) {
    findings.push({ level: 'warning', message: `${target}: aria-label 不在` })
  } else {
    findings.push({ level: 'info', message: `templates aria-label OK` })
  }
  ...
  console.log(`\n=== Findings (templates-page-main-aria-label-iter982) ===`)
