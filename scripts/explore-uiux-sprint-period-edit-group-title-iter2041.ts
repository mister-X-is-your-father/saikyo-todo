/**
 * Phase 6.15 loop iter2041: sprints-panel sprint-period edit form group に title 付与
 * (5 entity operations group family の続編)。
 */
...
async function main() {
  ...
  if (!sp.includes('iter2041') || !sp.includes('Sprint「${sprint.name}」の期間編集 form 操作')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'sprint-period edit form group title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — sprint-period edit form group title 付与、iter2037-1777 invariant 不変')
  } else {
