/**
 * playwright-iter1387 (mode-D 探索): light theme + 実際に item が render される 主要 view
 ...
 * 実 item 描画状態で light contrast を再検証する。
 *
 * 経路 B。実行: pnpm tsx --env-file=.env.local scripts/explore-uiux-axe-views-populated-light-iter1387.ts
 */
...
void runExplore({
  name: 'axe-views-populated-light-iter1387',
  ...
  body: async ({ page, workspaceId, findings }) => {
    for (const v of ['today', 'inbox', 'kanban', 'backlog', 'gantt', 'dashboard']) {
      ...
      await scan(page, `light ${v}`, findings)
    }
  },
})
