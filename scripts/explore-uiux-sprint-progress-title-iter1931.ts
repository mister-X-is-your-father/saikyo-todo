/**
 * Phase 6.15 loop iter1931: sprints-panel progressbar に title 付与
...
async function main() {
  ...
  if (!sp.includes('title={`Sprint「${sprint.name}」完了率 ${pct}% —')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'sprints-panel progressbar title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — sprints-panel progressbar title 付与、iter1929-1777 invariant 不変')
  } else {
