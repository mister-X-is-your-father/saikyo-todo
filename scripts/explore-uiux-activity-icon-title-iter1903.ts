/**
 * Phase 6.15 loop iter1903: activity-log action icon chip に title 付与
 * (iter1901 activity hint と同 status chip family pattern)。
 */
...
async function main() {
  ...
  if (!activity.includes('title={`${label} — 操作種別`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'activity-log action icon title が無い',
    })
  }
  if (!activity.includes('title={`${hint.label} — Activity 状態`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'iter1901 activity hint title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — activity-log icon title 付与、iter1901-1777 invariant 不変')
  } else {
