/**
 * Phase 6.15 loop iter1921: sprint-retro 計画vs納品 grid に title 付与
...
async function main() {
  ...
  if (!retro.includes('title={`計画 vs 納品 —')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'sprint-retro 計画vs納品 grid title が無い',
    })
  }
  ...
  if (!top.includes('title={`${label} — 合計`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'iter1919 top-items 合計 title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — sprint-retro 計画vs納品 grid title 付与、iter1919-1777 invariant 不変')
  } else {
