/**
 * Phase 6.15 loop iter2061: mock-timesheet ログイン form に title 付与
 * (7 form family の続編)。
 */
...
async function main() {
  ...
  if (!mlf.includes('title="Mock Timesheet ログインフォーム"')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'mock-login form title が無い',
    })
  }
  ...
  if (!sp.includes('title="Sprint デフォルト設定 編集フォーム"')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'iter2059 Sprint デフォルト編集フォーム title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — mock-login form title 付与、iter2059-1777 invariant 不変')
  } else {
