/**
 * Phase 6.15 loop iter2049: weekly-insight-widget region に title 付与
 * (6 region landmark hover summary、taskchute iter2047 同 pattern)。
 */
...
async function main() {
  ...
  if (!weekly.includes('iter2049') || !weekly.includes('title={`週次 Insight —')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'weekly-insight-widget region title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — weekly-insight-widget region title 付与、iter2047-1777 invariant 不変')
  } else {
