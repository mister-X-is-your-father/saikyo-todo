/**
 * Phase 6.15 loop iter1811: template-create + wf-create button に title 付与
 * (iter1809 sprint/goal create と同 pattern を Template/Workflow create にも展開、
 * creation form submit UX 全 5 entity (Workspace/Sprint/Goal/Template/Workflow) +
 * Time Entry の hover disclosure 完備)。
 *
 * 発見した UX gap (sighted only):
 *   - src/components/template/templates-panel.tsx template create button:
 *     conditional 3 path、no title
 *   - src/components/workflow/workflows-panel.tsx wf-create-btn:
 *     conditional 3 path、no title
 ...
 *
 * 実行: pnpm tsx scripts/explore-uiux-template-workflow-create-title-iter1811.ts
...
async function main() {
  ...
  const templatePanel = readFileSync(
    resolve(here, '../src/components/template/templates-panel.tsx'),
    'utf8',
  )
  ...
  // --- 1. template-create conditional 3 path text 維持 ---
  for (const t of [
    "'作成 — Template を作成するには名前を入力してください'",
    "'作成 — Template を作成中…'",
    "'作成 — Template を新規作成 (Cmd/Ctrl+Enter でも可)'",
  ]) {
    if (!templatePanel.includes(t)) {
      findings.push({
        level: 'error',
        source: 'a11y',
        message: `template-create conditional path text ${t} が消えている`,
      })
    }
  }

  // --- 2. wf-create conditional 3 path text 維持 ---
  for (const t of [
    ...
    if (!wfPanel.includes(t)) {
      findings.push({
        level: 'error',
        source: 'a11y',
        message: `wf-create conditional path text ${t} が消えている`,
      })
    }
  }

  // --- 3. wf-create-btn data-testid 維持 ---
  if (!wfPanel.includes('data-testid="wf-create-btn"')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'wf-create-btn data-testid が消えている',
    })
  }

  // --- 4. iter1809 sprint-create title 維持 ---
  ...
  if (findings.length === 0) {
    console.log(
      '(なし) — template-create + wf-create button に title 付与で creation 5 entity hover disclosure 完備、iter1809-1777 invariant 不変',
    )
  } else {
