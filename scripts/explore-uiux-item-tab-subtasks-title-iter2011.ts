/**
 * Phase 6.15 loop iter2011: item-edit-dialog tab-subtasks に title 付与
 * (tab-base iter2007 / tab-summary iter2009 と pair、6 tab sweep の 3 個目)。
 */
...
async function main() {
  ...
  if (!ied.includes('iter2011') || !ied.includes('子タスクタブ — 進捗')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'item-edit-dialog tab-subtasks title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — item-edit-dialog tab-subtasks title 付与、iter2009-1777 invariant 不変')
  } else {
