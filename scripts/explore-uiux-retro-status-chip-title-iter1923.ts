/**
 * Phase 6.15 loop iter1923: sprint-retro-status-chip に title 付与
...
async function main() {
  ...
  if (!retro.includes('title={`${label} ${count} 件 — ${toneLabel}`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'sprint-retro-status-chip title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — sprint-retro-status-chip title 付与、iter1921-1777 invariant 不変')
  } else {
