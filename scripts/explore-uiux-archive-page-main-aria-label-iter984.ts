/**
 * Phase 6.15 loop iter 984 — archive/page.tsx main aria-label。workspace sweep 8/8 完備。
 */
...
async function main(): Promise<void> {
  ...
  if (!/aria-label="アーカイブ済 Item 一覧"/.test(src)) {
    findings.push({ level: 'warning', message: `${target}: aria-label 不在` })
  } else {
    findings.push({ level: 'info', message: `archive aria-label OK` })
  }
  ...
  console.log(`\n=== Findings (archive-page-main-aria-label-iter984) ===`)
