/**
 * Phase 6.15 loop iter1937: items-board filter-must checkbox に title 付与
...
async function main() {
  ...
  if (
    ...
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'items-board filter-must checkbox title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — items-board filter-must title 付与、iter1935-1777 invariant 不変')
  } else {
