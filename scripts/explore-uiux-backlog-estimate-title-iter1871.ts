/**
 * Phase 6.15 loop iter1871: backlog-view estimate-summary chip に title 付与
 * (palette/taskchute/period/inbox/kanban title sweep の続編)。
 */
...
async function main() {
  ...
  if (!backlog.includes('title={`${estimateSummary} — Backlog 見積サマリ`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'backlog estimate-summary title が無い',
    })
  }
  ...
  if (!taskchute.includes('title={`P${item.priority ?? 4}`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'iter1865 taskchute priority title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — backlog estimate-summary title 付与、iter1869-1777 invariant 不変')
  } else {
