/**
 * Phase 6.15 loop iter1981: sprints-panel sprint-goal textarea に title 付与
 * (sprint-name iter1979 と pair、7 state-dependent input family の続編)。
 */
...
async function main() {
  ...
  if (!sp.includes('iter1981') || !sp.includes('Sprint ゴール (任意、最大 500 文字')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'sprints-panel sprint-goal textarea title が無い',
    })
  }
  if (!sp.includes('iter1979')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'iter1979 sprint-name title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — sprints-panel sprint-goal title 付与、iter1979-1777 invariant 不変')
  } else {
