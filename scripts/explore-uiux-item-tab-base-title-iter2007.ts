/**
 * Phase 6.15 loop iter2007: item-edit-dialog tab-base に title 付与
 * (sub-section nav iter1779 と同 hover context pattern、6 tab sweep の 1 個目)。
 */
...
async function main() {
  ...
  if (!ied.includes('title="基本タブ — タイトル / 状態 / 期限 / MUST / 担当 / Tag / DoD を編集"')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'item-edit-dialog tab-base title が無い',
    })
  }
  ...
  if (!pdca.includes('title={`集計期間 — 現在 ${days} 日、30 / 90 から選択`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'iter2005 pdca 集計期間 group title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — item-edit-dialog tab-base title 付与、iter2005-1777 invariant 不変')
  } else {
