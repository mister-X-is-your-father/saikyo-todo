/**
 * Phase 6.15 loop iter1959: calendar-view date navigation group に title 付与
 * (op-board iter1943 / inbox-region iter1945 と同 region/group summary pattern)。
 */
...
async function main() {
  ...
  if (!cal.includes('title={`カレンダー日付ナビゲーション —')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'calendar-view nav group title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — calendar-view nav group title 付与、iter1957-1777 invariant 不変')
  } else {
