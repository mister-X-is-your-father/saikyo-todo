/**
 * Phase 6.15 loop iter1927: cycle-check-stats-card ステータス分布 grid に title 付与
 * (iter1925 cycle Lead time grid と同 file 内 sweep)。
 */
...
async function main() {
  ...
  if (!cycle.includes('title={`ステータス分布 —')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'cycle-check ステータス分布 grid title が無い',
    })
  }
  if (!cycle.includes('title={`Lead time 統計 —')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'iter1925 cycle-check Lead time grid title が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — cycle-check ステータス分布 grid title 付与、iter1925-1777 invariant 不変')
  } else {
