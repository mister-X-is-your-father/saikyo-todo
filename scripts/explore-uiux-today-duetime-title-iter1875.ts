/**
 * Phase 6.15 loop iter1875: today-view dueTime chip に title 付与
 * (backlog DragHandle / estimate / kanban child-count / palette priority
 * title sweep の続編)。
 */
...
async function main() {
  ...
  if (!today.includes('title={`${it.dueTime.slice(0, 5)} — 期限時刻`}')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'today-view dueTime chip title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — today dueTime title 付与、iter1873-1777 invariant 不変')
  } else {
