/**
 * Phase 6.15 loop iter1740: workflows-panel CardTitle に title 付与で sighted hover で全
 * workflow name disclose (iter1739 sprints/goals と同 pattern を workflow card に展開)。
 *
 * 発見した UX gap (sighted only):
 *   src/components/workflow/workflows-panel.tsx CardTitle (line 313+) は
 *   `className="truncate text-base"` で長 workflow name 切れ、aria-label 無し
 *   (textContent が SR label)、sighted は hover で全 name 見れない。
 *
 * 修正 (src/components/workflow/workflows-panel.tsx, 1 line + 3 line comment):
 *   CardTitle に `title={wf.name}` 付与。className / id / role / aria-level / textContent
 *   完全不変、shadcn 編集なし、機能追加なし。
 *
 * 実行: pnpm tsx scripts/explore-uiux-workflow-card-title-iter1740.ts
...
async function main() {
  ...
  // --- 1. workflows-panel CardTitle に title={wf.name} 付与済 ---
  if (!wfPanel.match(/id=\{`wf-card-heading-\$\{wf\.id\}`\}[\s\S]{0,300}title=\{wf\.name\}/)) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'workflows-panel.tsx CardTitle に title={wf.name} が無い',
    })
  }

  // --- 2. truncate className 維持 ---
  if (!wfPanel.includes('className="truncate text-base"')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'workflows-panel.tsx CardTitle truncate className が消えている',
    })
  }

  // --- 3. role="heading" + aria-level={3} 維持 ---
  if (!wfPanel.match(/role="heading"\s*\n?\s*aria-level=\{3\}/)) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'workflows-panel.tsx CardTitle role/aria-level が消えている',
    })
  }
  ...
  if (findings.length === 0) {
    console.log(
      '(なし) — workflows-panel CardTitle に title 付与で sighted hover で全 wf name disclose、iter1739 / iter1738 / iter1737 / iter1734 / iter1732 invariant 不変',
    )
  } else {
