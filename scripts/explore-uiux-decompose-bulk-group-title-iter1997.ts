/**
 * Phase 6.15 loop iter1997: decompose-proposals bulk group に title 付与
 * (view-switcher iter1991 / filter group iter1993 と同 group landmark summary pattern)。
 */
...
async function main() {
  ...
  if (
    ...
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'decompose-proposals bulk group title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — decompose-proposals bulk group title 付与、iter1995-1777 invariant 不変')
  } else {
