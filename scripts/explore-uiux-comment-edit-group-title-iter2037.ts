/**
 * Phase 6.15 loop iter2037: comment-thread edit operations group に title 付与
 * (workflows/sprints/goals operations group iter2031-2035 と同 pattern)。
 */
...
async function main() {
  ...
  if (!ct.includes('iter2037')) {
    findings.push({
      level: 'error',
      source: 'a11y',
      message: 'comment-thread edit operations group title が無い',
    })
  }
  ...
  if (findings.length === 0) {
    console.log('(なし) — comment edit operations group title 付与、iter2035-1777 invariant 不変')
  } else {
