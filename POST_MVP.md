...
**これが MVP 後の 1 本目**。MVP 受け入れ基準を満たしたら、他項目より先にこれの
要件定義 → 設計 → 実装に入る。

- 目的: Item に対する実作業時間を記録・集計し、見積/実績ギャップを可視化する
- 要件定義は別途 `docs/spec-time-entries.md` (新設予定) で整理する
- 想定スコープ (要件定義で確定):
  - 作業記録エントリ (開始/終了 or duration / 対象 Item / メモ / 担当者)
  - 日次・週次集計 UI
  - Item 見積との突合 (見積列は MVP に無いので要件定義時に追加検討)
  - レポート出力 (CSV / workspace 月次)
- 未決事項 (要件定義で決める):
  - データモデル: `time_entries` 新規テーブル or `audit_log` 拡張
  - 入力粒度: 1 タスク 1 エントリ / タイマー連続記録 / 事後一括入力のどれを主にするか
  - Agent 稼働の扱い (token/cost は別軸管理済 → 対象外でよいか)
  - 承認フロー (上長承認 / 自己完結)
  - 外部勤怠システムとの連携 (最初は不要?)
- 依存: MVP の Item / Workspace / Auth / RLS がそのまま使える想定
