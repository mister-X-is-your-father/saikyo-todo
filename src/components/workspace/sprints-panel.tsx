        ...
        <EmptyState
          ...
          action={
            <button
              ...
              aria-label="Sprint 作成フォームへ移動 (『名前』入力欄にフォーカス)"
              onClick={() => {
          ...
          >
            ...
            {(status === 'planning' || status === 'active') && (
              <Button
                ...
                aria-label={
                  premortemPending
                    ? `Sprint「${sprint.name}」Pre-mortem 生成中… (PM Agent が想定リスクと早期警報を Doc 化)`
                    : sprint.premortemGeneratedAt
                      ? `Sprint「${sprint.name}」Pre-mortem 再生成 (PM Agent が想定リスクと早期警報を Doc 化、既存を上書き)`
                      : `Sprint「${sprint.name}」Pre-mortem 生成 (PM Agent が想定リスクと早期警報を Doc 化)`
                }
              >
                <Sparkles className="mr-1 h-3.5 w-3.5" aria-hidden="true" />
                <span aria-hidden="true">
