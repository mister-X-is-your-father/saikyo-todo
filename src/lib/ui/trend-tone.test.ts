...
describe('trendToneClass: positive polarity (default)', () => {
  it('up → blue', () => {
    expect(trendToneClass('up')).toBe('bg-blue-50 text-blue-700 border-blue-200')
    expect(trendToneClass('up', 'positive')).toBe('bg-blue-50 text-blue-700 border-blue-200')
  })

  it('down → red', () => {
    expect(trendToneClass('down')).toBe('bg-red-50 text-red-700 border-red-200')
  })
...
describe('trendToneClass: negative polarity (cost / error 軸)', () => {
  it('up → amber (warning)', () => {
    expect(trendToneClass('up', 'negative')).toBe('bg-amber-50 text-amber-700 border-amber-200')
  })
