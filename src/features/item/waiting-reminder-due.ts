...
// iter1024 refactor: 旧 local `MS_PER_DAY = 24 * 60 * 60 * 1000` を `@/lib/date/iso#MS_PER_DAY`
// に集約 (= iter360 で確立した「1 source of truth」 sweep)。
...
export function classifyReminderDue(state: WaitingForState, now: Date): ReminderDueDecision {
  ...
  const cadenceMs = state.reminderCadenceDays * MS_PER_DAY
  ...
  const daysElapsed = Math.floor((now.getTime() - requestedAt.getTime()) / MS_PER_DAY)
  return {
    kind: 'send',
